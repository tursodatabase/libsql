...
func runCounterExample(dbPath string) {
	...
	incCounterStatement := "INSERT INTO counter(country, city, value) VALUES('PL', 'WAW', 1) ON CONFLICT DO UPDATE SET value = IFNULL(value, 0) + 1 WHERE country = 'PL' AND city = 'WAW'"
	exec(db, incCounterStatement)
	exec(db, incCounterStatement)
