...
pub mod replication_log_server {
    ...
    #[async_trait]
    pub trait ReplicationLog: std::marker::Send + std::marker::Sync + 'static {
        async fn hello(
            &self,
    ...
    #[derive(Debug)]
    pub struct ReplicationLogServer<T> {
        inner: Arc<T>,
    ...
    impl<T> Clone for ReplicationLogServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "wal_log.ReplicationLog";
    impl<T> tonic::server::NamedService for ReplicationLogServer<T> {
