/*
** Name:        aegis128x4_common.h
** Purpose:     Common implementation for AEGIS-128x4
** Copyright:   (c) 2023-2024 Frank Denis
** SPDX-License-Identifier: MIT
*/

#define AEGIS_RATE      128
#define AEGIS_ALIGNMENT 64
...
AEGIS_init(const uint8_t *key, const uint8_t *nonce, AEGIS_AES_BLOCK_T *const state)
{
    ...
    for (i = 0; i < 10; i++) {
        ...
        state[7] = AEGIS_AES_BLOCK_XOR(state[7], context);
...
static void
AEGIS_state_init(aegis128x4_state *st_, const uint8_t *ad, size_t adlen, const uint8_t *npub,
           const uint8_t *k)
{
    AEGIS_BLOCKS             blocks;
...
static int
AEGIS_state_encrypt_update(aegis128x4_state *st_, uint8_t *c, size_t clen_max, size_t *written,
...
static int
AEGIS_state_encrypt_detached_final(aegis128x4_state *st_, uint8_t *c, size_t clen_max, size_t *written,
...
static int
AEGIS_state_encrypt_final(aegis128x4_state *st_, uint8_t *c, size_t clen_max, size_t *written,
                    size_t maclen)
{
    AEGIS_BLOCKS             blocks;
    AEGIS_STATE *const st =
...
static int
AEGIS_state_decrypt_detached_update(aegis128x4_state *st_, uint8_t *m, size_t mlen_max, size_t *written,
...
static int
AEGIS_state_decrypt_detached_final(aegis128x4_state *st_, uint8_t *m, size_t mlen_max, size_t *written,
...
static void
AEGIS_state_mac_init(aegis128x4_mac_state *st_, const uint8_t *npub, const uint8_t *k)
{
    AEGIS_BLOCKS                blocks;
...
static int
AEGIS_state_mac_update(aegis128x4_mac_state *st_, const uint8_t *ad, size_t adlen)
{
    AEGIS_BLOCKS             blocks;
    AEGIS_MAC_STATE *const st =
        (AEGIS_MAC_STATE *) ((((uintptr_t) &st_->opaque) + (AEGIS_ALIGNMENT - 1)) &
...
static int
AEGIS_state_mac_final(aegis128x4_mac_state *st_, uint8_t *mac, size_t maclen)
{
    AEGIS_BLOCKS             blocks;
    AEGIS_MAC_STATE *const st =
        (AEGIS_MAC_STATE *) ((((uintptr_t) &st_->opaque) + (AEGIS_ALIGNMENT - 1)) &
...
static void
AEGIS_state_mac_reset(aegis128x4_mac_state *st_)
{
    AEGIS_MAC_STATE *const st =
        (AEGIS_MAC_STATE *) ((((uintptr_t) &st_->opaque) + (AEGIS_ALIGNMENT - 1)) &
                                  ~(uintptr_t) (AEGIS_ALIGNMENT - 1));
...
static void
AEGIS_state_mac_clone(aegis128x4_mac_state *dst, const aegis128x4_mac_state *src)
{
