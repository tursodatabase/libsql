...
where
    ...
    F: MakeService<(), Vec<Query>, MakeError = Error> + Sync,
    F::Future: 'static + Send,
    F::Service:
        Service<Vec<Query>, Response = Vec<Option<QueryResult>>, Error = Error> + Sync + Send,
    <F::Service as Service<Vec<Query>>>::Future: Send,
{
    type Response = ();
    type Error = Error;
