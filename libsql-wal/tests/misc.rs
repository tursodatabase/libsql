...
fn transaction_rollback() {
    ...
    assert!(conn2.query_row("select count(0) from test", (), |_| {
        Ok(())
    }).is_err());
    ...
    assert!(conn1.query_row("select count(0) from test", (), |_| {
        Ok(())
    }).is_err());
    ...
    conn1.query_row("select count(0) from test", (), |r| {
        assert_eq!(r.get::<_, usize>(0).unwrap(), 0);
        Ok(())
    }).unwrap();
    conn2.query_row("select count(0) from test", (), |r| {
        assert_eq!(r.get::<_, usize>(0).unwrap(), 0);
        Ok(())
    }).unwrap();
}
